...
template <typename T> inline T equalSumCrossfade(T a, T b, T p) { return a * (0.5f * (1.f - p)) + b * (0.5f * (1.f + p)); }

// equal power crossfade, -1 <= p <= 1
template <typename T> inline T equalPowerCrossfade(T a, T b, const T p) {
    // return std::min(std::exp(4.f * p), 1.f) * b + std::min(std::exp(4.f * -p), 1.f) * a;
    return simd::fmin(exponentialBipolar80Pade_5_4(p + 1), 1.f) * b + simd::fmin(exponentialBipolar80Pade_5_4(1 - p), 1.f) * a;
}
...
struct Morphader : Module {
    ...
    enum ParamIds {
        CV_PARAM,
        ENUMS(A_LEVEL, NUM_MIXER_CHANNELS),
        ENUMS(B_LEVEL, NUM_MIXER_CHANNELS),
        ENUMS(MODE, NUM_MIXER_CHANNELS),
        FADER_LAG_PARAM,
        FADER_PARAM,
        NUM_PARAMS
    };
    enum InputIds {
        ENUMS(CV_INPUT, NUM_MIXER_CHANNELS),
        ENUMS(A_INPUT, NUM_MIXER_CHANNELS),
        ENUMS(B_INPUT, NUM_MIXER_CHANNELS),
        NUM_INPUTS
    };
    enum OutputIds {
        ENUMS(OUT, NUM_MIXER_CHANNELS),
        NUM_OUTPUTS
    };

    enum LightIds {
        ENUMS(A_LED, NUM_MIXER_CHANNELS * 3),
        ENUMS(B_LED, NUM_MIXER_CHANNELS * 3),
        NUM_LIGHTS
    };
    enum CrossfadeMode {
        AUDIO_MODE,
    ...
    float computeMasterCrossfade(const float deltaTime) {
        const float slewLambda = 2.0f / params[FADER_LAG_PARAM].getValue();
        slewLimiter.setSlew(slewLambda);
        return slewLimiter.process(deltaTime, params[FADER_PARAM].getValue());
    }

    // compute average of the 4 x float_4 crossfades for mono output, only counting up to numChannels
    float computeChannelCrossfadeMono(const float_4 crossfadesPoly[4], const int numChannels) {
        float sum = 0.f;
        for (int c = 0; c < numChannels; c++) {
            sum += crossfadesPoly[c / 4][c % 4];
        }
        return sum / static_cast<float>(numChannels);
    }

    float_4 computeChannelCrossfadePoly(const int channel, const int c, const float masterCrossfadeValue) {
        if (channel == 0) {
            const float_4 crossfadeCV = simd::clamp(inputs[CV_INPUT + channel].getPolyVoltageSimd<float_4>(c), 0.f, 10.f);
            const float_4 channelCrossfade = params[CV_PARAM].getValue() * (crossfadeCV * 0.2f) + masterCrossfadeValue;
            return simd::clamp(channelCrossfade, -1.f, +1.f);
        }

        if (inputs[CV_INPUT + channel].isConnected()) {
            const float_4 crossfadeCV = simd::clamp(inputs[CV_INPUT + channel].getPolyVoltageSimd<float_4>(c), 0.f, 10.f);
            return simd::clamp(crossfadeCV * 0.2f - 1.f, -1.f, +1.f);
        }

        if (inputs[CV_INPUT + 0].isConnected()) {
            const float_4 crossfadeCV = simd::clamp(inputs[CV_INPUT + 0].getPolyVoltageSimd<float_4>(c), 0.f, 10.f);
            const float_4 channelCrossfade = params[CV_PARAM].getValue() * (crossfadeCV * 0.2f) + masterCrossfadeValue;
            return simd::clamp(channelCrossfade, -1.f, +1.f);
        }

        return simd::clamp(float_4(masterCrossfadeValue), -1.f, +1.f);
    }
    ...
    void process(const ProcessArgs &args) override {
        ...
        int maxChannels = 1;
        float_4 mix[4] = {};
        float_4 channelCrossFades[4] = {};
        const float masterCrossfadeValue = computeMasterCrossfade(args.sampleTime);
        ...
        for (int i = 0; i < NUM_MIXER_CHANNELS; i++) {
            ...
            for (int c = 0; c < channels; c += 4) {
                ...
                channelCrossFades[c / 4] = computeChannelCrossfadePoly(i, c, masterCrossfadeValue);

                switch (static_cast<CrossfadeMode>(params[MODE + i].getValue())) {
                case CV_MODE: {
                    out[c / 4] = equalSumCrossfade(inA, inB, channelCrossFades[c / 4]);
                    break;
                }
                case AUDIO_MODE: {
                    ...
                    out[c / 4] = equalPowerCrossfade(inA, inB, channelCrossFades[c / 4]);
                    break;
                }
                default: {
                    out[c / 4] = 0.f;
            ...
            const float channelCrossfadeMono = computeChannelCrossfadeMono(channelCrossFades, channels);

            switch (static_cast<CrossfadeMode>(params[MODE + i].getValue())) {
            case AUDIO_MODE: {
                float brightnessA = equalPowerCrossfade(1.f, 0.f, channelCrossfadeMono);
                float brightnessB = equalPowerCrossfade(0.f, 1.f, channelCrossfadeMono);
                // red LEDs for audio mode
                if (channels == 1) {
                    lights[A_LED + 3 * i].setBrightness(brightnessA);
                    lights[B_LED + 3 * i].setBrightness(brightnessB);
                    lights[A_LED + 3 * i + 1].setBrightness(0.f);
                    lights[B_LED + 3 * i + 1].setBrightness(0.f);
                    lights[A_LED + 3 * i + 2].setBrightness(0.f);
                    lights[B_LED + 3 * i + 2].setBrightness(0.f);
                } else {
                    // blue LEDs for polyphonic
                    lights[A_LED + 3 * i].setBrightness(0.f);
                    lights[B_LED + 3 * i].setBrightness(0.f);
                    lights[A_LED + 3 * i + 1].setBrightness(0.f);
                    lights[B_LED + 3 * i + 1].setBrightness(0.f);
                    lights[A_LED + 3 * i + 2].setBrightness(brightnessA);
                    lights[B_LED + 3 * i + 2].setBrightness(brightnessB);
                }
                break;
            }
            case CV_MODE: {
                float brightnessA = equalSumCrossfade(1.f, 0.f, channelCrossfadeMono);
                float brightnessB = equalSumCrossfade(0.f, 1.f, channelCrossfadeMono);
                if (channels == 1) {
                    // red LEDs for mono
                    lights[A_LED + 3 * i + 0].setBrightness(brightnessA);
                    lights[B_LED + 3 * i + 0].setBrightness(brightnessB);
                    lights[A_LED + 3 * i + 1].setBrightness(0.f);
                    lights[B_LED + 3 * i + 1].setBrightness(0.f);
                    lights[A_LED + 3 * i + 2].setBrightness(0.f);
                    lights[B_LED + 3 * i + 2].setBrightness(0.f);
                    ...
                    // blue LEDs for polyphonic
                    lights[A_LED + 3 * i + 0].setBrightness(0.f);
                    lights[B_LED + 3 * i + 0].setBrightness(0.f);
                    lights[A_LED + 3 * i + 1].setBrightness(0.f);
                    lights[B_LED + 3 * i + 1].setBrightness(0.f);
                    lights[A_LED + 3 * i + 2].setBrightness(brightnessA);
                    lights[B_LED + 3 * i + 2].setBrightness(brightnessB);
...
struct MorphaderWidget : ModuleWidget {
    MorphaderWidget(Morphader *module) {
        ...
        addChild(
            createLightCentered<SmallLight<RedGreenBlueLight>>(mm2px(Vec(37.594, 24.378)), module, Morphader::A_LED + 3 * 0));
        addChild(
            createLightCentered<SmallLight<RedGreenBlueLight>>(mm2px(Vec(37.594, 41.908)), module, Morphader::A_LED + 3 * 1));
        addChild(
            createLightCentered<SmallLight<RedGreenBlueLight>>(mm2px(Vec(37.594, 59.488)), module, Morphader::A_LED + 3 * 2));
        addChild(
            createLightCentered<SmallLight<RedGreenBlueLight>>(mm2px(Vec(37.594, 76.918)), module, Morphader::A_LED + 3 * 3));

        addChild(
            createLightCentered<SmallLight<RedGreenBlueLight>>(mm2px(Vec(45.332, 24.378)), module, Morphader::B_LED + 3 * 0));
        addChild(
            createLightCentered<SmallLight<RedGreenBlueLight>>(mm2px(Vec(45.332, 41.908)), module, Morphader::B_LED + 3 * 1));
        addChild(
            createLightCentered<SmallLight<RedGreenBlueLight>>(mm2px(Vec(45.332, 59.488)), module, Morphader::B_LED + 3 * 2));
        addChild(
            createLightCentered<SmallLight<RedGreenBlueLight>>(mm2px(Vec(45.332, 76.918)), module, Morphader::B_LED + 3 * 3));
    }
};
