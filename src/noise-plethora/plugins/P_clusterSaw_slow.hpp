...
class clusterSaw_slow : public NoisePlethoraPlugin {
...
public:
	...
	~clusterSaw_slow() override {}

	clusterSaw_slow(const clusterSaw_slow&) = delete;
	clusterSaw_slow& operator=(const clusterSaw_slow&) = delete;
	...
	float processGraph(float sampleTime) override {
		...
		const float mix1 = mixer1.process(
		                     waveform1.process(sampleTime),
		                     waveform2.process(sampleTime),
		                     waveform3.process(sampleTime),
		                     waveform4.process(sampleTime));

		const float mix2 = mixer2.process(
		                     waveform5.process(sampleTime),
		                     waveform6.process(sampleTime),
		                     waveform7.process(sampleTime),
		                     waveform8.process(sampleTime));

		const float mix3 = mixer3.process(
		                     waveform9.process(sampleTime),
		                     waveform10.process(sampleTime),
		                     waveform11.process(sampleTime),
		                     waveform12.process(sampleTime));

		float w16 = waveform16.process(sampleTime);
		const float mix4 = mixer4.process(
		                     waveform13.process(sampleTime),
		                     waveform14.process(sampleTime),
		                     waveform15.process(sampleTime),
		                     w16);
...
private:
	...
	AudioSynthWaveformModulatedFloat waveform4;      //xy=378.00000381469727,851.0000076293945
	AudioSynthWaveformModulatedFloat waveform2;      //xy=380.00000381469727,746.0000076293945
	AudioSynthWaveformModulatedFloat waveform3;      //xy=380.00000381469727,791.0000076293945
	AudioSynthWaveformModulatedFloat waveform1;      //xy=381.00000381469727,703.0000076293945
...
REGISTER_PLUGIN(clusterSaw_slow);
