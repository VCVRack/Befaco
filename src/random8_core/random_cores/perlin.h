...
extern "C"
{
    ...
    //==================1D=====================
    static inline int noise1(int x, int seed = 0)
    {
        int tmp = hash[seed % 256];
        return hash[(tmp + x) % 256];
    }

    static inline float noise1d(float x)
    {
        int x_int = x;
    ...
    static inline float perlin1d(float x, float freq, int depth, float amplitude = 1.0)
    {
        float xa = x * freq;
        float amp = amplitude;
        float fin = 0;
    ...
    static inline int noise2(int x, int y, int seed = 0)
    {
    ...
    static inline float perlin2d(float x, float y, float freq, int depth)
    {
        float xa = x * freq;
        float ya = y * freq;
        float amp = 1.0;
